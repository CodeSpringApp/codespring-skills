---
name: cs-website-foundation
description: Use when an app needs a clear marketing website plan and a founder-facing NOW/NEXT/LATER workboard. Maps every page to purpose, proof, CTA and next action.
metadata:
  author: codespring
  version: "0.1"
---
...
For every page capture: URL, page type, audience, job to be done, primary CTA, proof, owner, status, dependency, success metric, next action and review date.
...
**Done when:** the inventory covers every route that matters to a buyer, customer or crawler, plus the missing pages required for the conversion path.
...
- Technical crawlability, canonical, sitemap and Search Console setup → `cs-seo-setup`.
- Answer-led, source-backed Resources content → `cs-aeo-content`.
- Recurring briefs, publishing, review and founder updates → `cs-content-operations`.
- Customer interviews and approved story production → `cs-content-operations` until a dedicated story/video skill is proven.
...
- [ ] Every row has a job, audience, CTA, owner, status and next action.
