...
```
cs-plan-app  or  cs-import-codebase        (map + notes exist)
        │
        ▼
   cs-ui-mockup   ← you are here
        │            build it, show it, feed corrections back
        ▼
  cs-create-prd → cs-create-tasks → cs-build-feature
```
...
- **No map** → wrong skill. `cs-plan-app` (no code) or `cs-import-codebase` (existing code) first.
...
**→ `cs-create-prd`**, now that the plan reflects something the client has actually seen.
Then `cs-create-tasks`, then `cs-build-feature`.
