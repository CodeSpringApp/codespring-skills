...
description: >
  ...
  note each — then PRDs (via cs-create-prd) and Kanban tasks. Takes an OPTIONAL
  audit + verdict: with no audit it documents what exists; with an audit it
  builds the corrected target map (what the app should be) and generates fix or
...
**No audit** → tasks come from the PRDs, not from findings. Hand off to **`cs-create-tasks`** for that; it owns PRD-derived task lists. This skill only writes the findings-derived tasks, because it is the only place the findings exist.
...
Report when it finishes and offer to fold the shortlist into the notes/PRDs, or hand off to `cs-resync-codebase`.
