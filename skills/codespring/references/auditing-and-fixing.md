...
|---|---|---|---|
| "get my app into CodeSpring", "map my app" | *Understand what I have* | `cs-import-codebase` | Document what exists |
| "my app is a mess, diagnose it and plan the fix" | *Tell me what's wrong and what to do* | `cs-audit-codebase` | Diagnose, then hand off |
...
`cs-audit-codebase` does not build the map itself. It produces findings + a verdict and hands both to `cs-import-codebase`. One code path builds maps.
...
Apply the same test to everything: *idempotent*, *RLS*, *IDOR*, *migration*, *ORM*, *schema drift*, *silent failure path*, *hydration*, *denormalised*. Either explain it inline or find the plain sentence. Full voice rules and structure for the deliverable live in `cs-audit-codebase/references/plain-english-writeup.md`.
...
2. **After the tasks are done, resync.** `cs-resync-codebase` is what closes the loop and makes the map describe reality again. Prompt for it when a feature's tasks all reach done.
...
7. **An existing map is not automatically a good map.** Run the map-quality ladder (`cs-import-codebase/references/target-map.md`) before trusting one. Never create a second CodeSpring project for the same app without explicit confirmation, and if there are already two, establish **which one is authoritative** before writing anything.
