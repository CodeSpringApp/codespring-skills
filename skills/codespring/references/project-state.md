...
|---|---|---|---|
| **Understand** | **context** — what is true now, and what it should be | audit findings, `FINDINGS.md`, core features + sub-features, one "how it works" note per core feature, tech stack | `cs-audit-codebase`, `cs-import-codebase` |
| **Plan** | **instructions** — what to build and in what order | PRDs (Frontend / Backend), numbered Kanban tasks with severities | `cs-create-prd`, `cs-create-tasks` |
| **Build** | **changed code** | commits, tasks moving to done | `cs-build-feature` |
| *(then)* | **truth again** | the map redrawn to match what was actually built | `cs-resync-codebase` |
...
| Linked, no features | Understand not started | Run the **five questions** below. All five yes → `cs-import-codebase`. Any no → `cs-audit-codebase`. No code yet → plan from scratch. |
| Features, no notes | Skeleton map only | Write the "how it works" note per core feature. Do not generate PRDs yet. |
| Features + notes, no PRDs, no tasks | **Understand done, Plan not started** | If not `audited` and the user came in wanting fixes → audit, then tasks. Otherwise → PRDs (`cs-create-prd`) then tasks (`cs-create-tasks`). |
| Features + notes + PRDs, no tasks | Plan half done | `cs-create-tasks`. |
| Tasks in `todo` | Ready to build | `cs-build-feature`. |
| Tasks mostly `done`, code has moved | Built | `cs-resync-codebase`. |
...
`PIA App (clean)`: **6 core features, 25 sub-features, 6 notes, 0 PRDs, 0 tasks.** The user runs `cs-audit-codebase`.
...
Used when the user doesn't know whether their app is "healthy" or "a mess", and by `cs-import-codebase` to decide whether to offer an audit before importing. Two minutes, read-only, no sub-agents:
