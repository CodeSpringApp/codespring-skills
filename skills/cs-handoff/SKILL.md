...
Run it **after** `cs-create-tasks`, once the map, notes, PRDs and tasks all exist.

Read: `codespring` skill `references/project-state.md` and `references/pitfalls.md`, and **`cs-audit-codebase/references/plain-english-writeup.md`** — the voice rules there apply here exactly. Third-grade reading level, no file paths, no jargon, no framework names.
