...
| `cs-agent-setup` | agent | dogfooding | 0.2 | Set up or repair a CodeSpring agent end to end: dashboard and environment orientation, stable model route, immutable revision, skills/tools/MCP, secure server/browser credentials, deployment keys and IDs, development-backed public betas, progressive generative UI and a replay-backed `AGENT_SETUP.md` receipt. Keeps the implementation technically exact while guiding non-technical AI builders one plain-English action at a time. Dogfooded on the BuildProof research agent; excludes provider-specific incident workarounds. |
...
| `dashboard-guide.md` | `cs-agent-setup` | A fifth-grade-reading-level tour of every CodeSpring Agents page and term, including provider keys vs model IDs vs voice profiles, updates to knowledge and skills, Playground, sessions, API keys, tools, MCP, local apps and the safe setup order. |
| `development-and-production.md` | `cs-agent-setup` | The practice-room/live-room explanation of development and production, the resources that do not copy between them, roles, localhost vs environment, publishing vs deployment, and the explicit promotion checklist. |
...
| `deployment-credentials-and-ids.md` | `cs-agent-setup` | Distinguishes CodeSpring production, hosting production and production-ready status; maps every key and ID to its dashboard location; and covers secure Vercel variables, development-backed public betas and common deployability failures. |
