...
| `cs-agent-setup` | agent | dogfooding | 0.1 | Set up or repair a CodeSpring agent end to end: dashboard and environment orientation, stable model route, immutable revision, skills/tools/MCP, secure server/browser credentials, progressive generative UI and a replay-backed `AGENT_SETUP.md` receipt. Keeps the implementation technically exact while guiding non-technical AI builders one plain-English action at a time. Dogfooded on the BuildProof research agent; excludes provider-specific incident workarounds. |
