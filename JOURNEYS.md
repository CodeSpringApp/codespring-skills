...
```
  cs-build-plan-app        the idea → core features, sub-features, notes, in CodeSpring
        ▼
  cs-build-ui-mockup       a clickable local mockup + style guide → review → corrections back into the plan
        ▼
  cs-build-create-prd      each feature → Frontend + Backend PRDs
        ▼
  cs-build-create-tasks    each feature → numbered, ordered, dependency-aware tasks
        ▼
  cs-build-handoff         the client pack: what we're building, the mockup, the plan link, what happens next
        ▼
  ─── they open it in CodeSpring, then in Ferb ───
        ▼
  cs-build-feature   they build, task by task
        ▼
  cs-build-resync-codebase keep the map honest as the code drifts from it
```
...
```
  cs-build-audit-codebase       is this any good? → plain-English findings + a rebuild-or-fix verdict
        ▼
  cs-build-import-codebase      the code → core features, sub-features, notes, in CodeSpring
                          (with an audit, it maps the CORRECTED TARGET and turns findings into fix tasks)
        ▼
  cs-build-create-prd           each feature → PRDs
        ▼
  cs-build-handoff              the client pack: what's wrong, what it costs, the plan to fix it
        ▼
  ─── they open it in CodeSpring, then in Ferb ───
        ▼
...
**`cs-build-ui-mockup` fits here too**, whenever the fix involves redesigning screens rather than just repairing logic.
...
| **`cs-build-plan-app`** | There's no code — just an idea, a call, a recording. |
| **`cs-build-audit-codebase`** | There's code and you need to know whether it's worth building on. |
| **`cs-build-import-codebase`** | There's code and you want it on the canvas as features and notes. |
| **`cs-build-ui-mockup`** | The map and notes exist and it has a user interface. **Before the PRDs.** |
| **`cs-build-create-prd`** | A feature has a good note and needs build instructions. |
| **`cs-build-create-tasks`** | A feature has PRDs and needs an ordered task list. |
...
**1. Is there code?** No → `cs-build-plan-app`. Yes → question 2.
...
**All five yes → `cs-build-import-codebase`. Any no → `cs-build-audit-codebase`.**
...
|---|---|---|
| Understand | `cs-build-plan-app` | `cs-build-audit-codebase` → `cs-build-import-codebase` |
| Show | `cs-build-ui-mockup` | `cs-build-ui-mockup` (if screens change) |
| Plan | `cs-build-create-prd` → `cs-build-create-tasks` | `cs-build-create-prd` → `cs-build-create-tasks` |
| Hand over | `cs-build-handoff` | `cs-build-handoff` |
